...
class TestTextMelLoader(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, dataset_entries, hparams, mixed_training=False, normalize=True):
        ...
        self.all_emotions = ["Neutral", "Surprise", "Happy", "Sad", "Angry"]

        random.seed(hparams.seed)
        random.shuffle(self.dataset_entries)

    def get_dataset_entry(self, dataset_entries):
        text, emotion_vector, audiopath, speaker = (
            dataset_entries[0],
            dataset_entries[1],
            dataset_entries[2],
            dataset_entries[3],
        )
        p_text = self.get_text(text)
        emotion_vector, emotion = self.get_vector(emotion_vector)

        input_mel = self.get_mel(self.audio_file_root + audiopath, 0)
        real_mel = input_mel

        return (p_text, input_mel, real_mel, emotion_vector, text, emotion, speaker)
    ...
    def get_vector(self, vector_string):
        ...
        emotion = self.all_emotions[vector_list.index(max(vector_list))]

        if self.mixed_training:
            vector_list[1] = 1.0

        return vector_list, emotion
...
class TestTextMelCollate:
    ...
    def __call__(self, batch):
        ...
        emotion = [x[5] for x in batch]
        speaker = [x[6] for x in batch]
