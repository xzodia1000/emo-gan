...
class EmotionClassifier(nn.Module):
    def __init__(self, hparams):
        ...
        self.num_classes = hparams.n_emotions
        self.L1 = hparams.classifier_l1
        self.L2 = hparams.classifier_l2
        self.cell_units = hparams.classifier_cell_units
        self.num_linear = hparams.classifier_num_linear
        self.p = hparams.classifier_p
        self.time_step = hparams.classifier_time_step
        self.F1 = hparams.classifier_F1
        self.dropout_prob = 1 - hparams.classifier_dropout
        ...
        self.conv2 = nn.Conv2d(
            self.L1, self.L2, (5, 3), padding=(2, 1)
        )  # [5, 3, 128, 256]
        self.conv3 = nn.Conv2d(
            self.L2, self.L2, (5, 3), padding=(2, 1)
        )  # [5, 3, 256, 256]
        ...
        self.rnn = nn.LSTM(
            input_size=self.num_linear,
            hidden_size=self.cell_units,
            batch_first=True,
            num_layers=1,
            bidirectional=True,
        )
    ...
    def forward(self, x):
        ...
        layer1 = F.max_pool2d(
            layer1, kernel_size=(2, 4), stride=(2, 4)
        )  # [1,2,4,1], padding = 'valid'
        ...
        layer6 = layer6.reshape(
            -1, self.time_step, self.L2 * self.p
        )  # (-1, 150, 256*10)
        ...
        outputs1, output_states1 = self.rnn(
            linear1
        )  # outputs1 : [10, 150, 128] (B,T,D)

        # # attention
        v = self.sigmoid(self.a_fc1(outputs1))  # (10, 150, 1)
        alphas = self.softmax(
            self.a_fc2(v).squeeze()
        )  # (B,T) shape, alphas are attention weights
